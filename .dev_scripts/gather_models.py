...
def get_final_results(
    log_json_path, epoch_or_iter, results_lut="coco/bbox_mAP", by_epoch=True
):
    result_dict = dict()
...
def find_last_dir(model_dir):
    ...
    for time_stamp in os.scandir(model_dir):
        if osp.isdir(time_stamp):
            dst_time = time.mktime(time.strptime(time_stamp.name, "%Y%m%d_%H%M%S"))
...
def convert_model_info_to_pwc(model_infos):
    ...
    for model in model_infos:
        ...
        link_string += "{}/{}".format(
            model["config"].rstrip(".py"), osp.split(model["model_path"])[-1]
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--best", action="store_true", help="whether to gather the best model."
    )
...
def main():
    ...
    for used_config in used_configs:
        ...
        if args.best is True:
            ...
            final_model = "{}_{}.pth".format(
                "epoch" if by_epoch else "iter", final_epoch_or_iter
            )
        ...
        latest_exp_json = osp.join(
            exp_dir, latest_exp_name, "vis_data", latest_exp_name + ".json"
        )
    ...
    for model in model_infos:
        ...
        trained_model_path = osp.join(
            models_root, model["config"], model["final_model"]
        )
