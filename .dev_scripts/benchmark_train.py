...
def fast_train_model(config_name, args, logger=None):
    ...
    if args.work_dir is not None:
        ...
        cfg.work_dir = osp.join(
            args.work_dir, osp.splitext(osp.basename(config_name))[0]
        )
    elif cfg.get("work_dir", None) is None:
        # use config filename as default work_dir if cfg.work_dir is None
        cfg.work_dir = osp.join(
            "./work_dirs", osp.splitext(osp.basename(config_name))[0]
        )
...
def main(args):
    ...
    logger = MMLogger.get_instance(
        name="MMLogger", log_file="benchmark_train.log", log_level=logging.ERROR
    )
