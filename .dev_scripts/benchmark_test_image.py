...
def inference_model(config_name, checkpoint, visualizer, args, logger=None):
    ...
    model = init_detector(cfg, checkpoint, palette=args.palette, device=args.device)
    ...
    if args.show or args.out_dir is not None:
        ...
        if args.out_dir is not None:
            ...
            out_file = osp.join(
                out_dir, config_name.split("/")[-1].replace("py", "jpg")
            )
...
def main(args):
    ...
    logger = MMLogger.get_instance(
        name="MMLogger", log_file="benchmark_test_image.log", log_level=logging.ERROR
    )
    ...
    for model_key in config:
        ...
        for model_info in model_infos:
            ...
            try:
                # build the model from a config file and a checkpoint file
                inference_model(config_name, checkpoint, visualizer, args, logger)
