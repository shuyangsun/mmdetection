...
def parse_args():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=1, help="The batch size during the inference."
    )
    parser.add_argument(
...
def inference(config_file, checkpoint, work_dir, args, exp_name):
    ...
    if args.flops:
        ...
        try:
            ...
            outputs = get_model_complexity_info(
                model, input_shape, data["inputs"], show_table=False, show_arch=False
            )
            flops = outputs["flops"]
            params = outputs["params"]
            activations = outputs["activations"]
...
def main(args):
    ...
    if args.models:
        patterns = [re.compile(pattern.replace("+", "_")) for pattern in args.models]
        filter_models = {}
    ...
    for model_name, model_info in tqdm(models.items()):
        ...
        try:
            ...
            result = inference(
                MMDET_ROOT / config, checkpoint, tmpdir.name, args, model_name
            )
            result["valid"] = "PASS"
