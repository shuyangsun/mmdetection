...
def parse_args():
    ...
    parser.add_argument(
        "--out", type=str, default="link_reports.txt", help="output path of reports"
    )
...
def check_link(
    match_tuple: MatchTuple, http_session: requests.Session, logger: logging = None
...
def check_url(
    match_tuple: MatchTuple, http_session: requests.Session
) -> Tuple[bool, str]:
...
def main():
    ...
    for resource_prefix in ("http://", "https://"):
        http_session.mount(
            ...
            requests.adapters.HTTPAdapter(
                max_retries=5, pool_connections=20, pool_maxsize=args.num_threads
            ),
        )
    ...
    with Pool(processes=args.num_threads) as pool:
        results = pool.starmap(
            check_link, [(match, http_session, logger) for match in list(all_matches)]
        )
