...
def download(url, out_file, min_bytes=math.pow(1024, 2), progress=True):
    ...
    assert_msg = (
        f"Downloaded url '{url}' does not exist " f"or size is < min_bytes={min_bytes}"
    )
    try:
        print(f"Downloading {url} to {out_file}...")
        torch.hub.download_url_to_file(url, str(out_file), progress=progress)
        assert osp.exists(out_file) and osp.getsize(out_file) > min_bytes, assert_msg
    except Exception as e:
        if osp.exists(out_file):
            os.remove(out_file)
        print(f"ERROR: {e}\nRe-attempting {url} to {out_file} ...")
        os.system(
            f"curl -L '{url}' -o '{out_file}' --retry 3 -C -"
...
def parse_args():
    ...
    parser.add_argument(
        "--intranet", action="store_true", help="switch to internal network url"
    )
