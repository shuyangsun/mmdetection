...
model = dict(
    ...
    roi_head=dict(bbox_head=dict(bbox_coder=dict(clip_border=True), num_classes=1)),
)
# data
data_root = "data/MOT20/"
train_dataloader = dict(
    dataset=dict(data_root=data_root, ann_file="annotations/train_cocoformat.json")
)
val_dataloader = dict(
    dataset=dict(data_root=data_root, ann_file="annotations/train_cocoformat.json")
)
...
param_scheduler = [
    ...
    dict(type="MultiStepLR", begin=0, end=8, by_epoch=True, milestones=[6], gamma=0.1),
]
