_base_ = [
    "./bytetrack_yolox_x_8xb4-80e_crowdhuman-mot17halftrain_" "test-mot17halfval.py"
]
...
train_pipeline = [
    dict(type="Mosaic", img_scale=img_scale, pad_val=114.0, bbox_clip_border=True),
    dict(
        type="RandomAffine",
    ...
    dict(type="Resize", scale=img_scale, keep_ratio=True, clip_object_border=True),
...
val_dataloader = dict(dataset=dict(ann_file="annotations/train_cocoformat.json"))

test_dataloader = dict(
    dataset=dict(data_root="data/MOT20", ann_file="annotations/test_cocoformat.json")
)
