...
class RefSegMetric(BaseMetric):
    ...
    def __init__(self, metric: Sequence = ("cIoU", "mIoU"), **kwargs):
        ...
        assert set(metric).issubset(
            ["cIoU", "mIoU"]
        ), f"Only support cIoU and mIoU, but got {metric}"
        assert len(metric) > 0, "metrics should not be empty"
        self.metrics = metric

    def compute_iou(self, pred_seg: torch.Tensor, gt_seg: torch.Tensor) -> tuple:
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            bs = len(pred_label)
            iou = overlap.reshape(bs, -1).sum(-1) * 1.0 / union.reshape(bs, -1).sum(-1)
