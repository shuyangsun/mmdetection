...
class CocoPanopticMetric(BaseMetric):
    ...
    def gt_to_coco_json(
        ...
        for gt_dict in gt_dicts:
            ...
            annotation = dict(
                image_id=img_id, segments_info=segments_info, file_name=segm_file
            )
    ...
    def _compute_batch_pq_stats(self, data_samples: Sequence[dict]):
        ...
        if self._coco_api is None:
            ...
            cat_ids = self._coco_api.get_cat_ids(cat_names=self.dataset_meta["classes"])
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        if self.tmp_dir is None:
            ...
            if self.format_only:
                logger.info(
                    "results are saved in " f"{osp.dirname(self.outfile_prefix)}"
                )
                return dict()
            ...
            gt_json = [
                {"image_id": k, "segments_info": v, "file_name": imgs[k]["segm_file"]}
