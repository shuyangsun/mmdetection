...
class SemSegMetric(BaseMetric):
    ...
    def __init__(
        ...
        if not set(iou_metrics).issubset(set(["mIoU", "mDice", "mFscore"])):
            raise KeyError(
                f"metrics {iou_metrics} is not supported. "
    ...
    def get_return_metrics(self, results: list) -> dict:
        ...
        for metric in self.metrics:
            if metric == "mIoU":
                ...
                f_value = torch.tensor(
                    [f_score(x[0], x[1], self.beta) for x in zip(precision, recall)]
                )
        ...
        ret_metrics = {
            metric: value.cpu().numpy() for metric, value in ret_metrics.items()
        }
...
def print_semantic_table(
    results: dict, class_names: list, logger: Optional[Union["MMLogger", str]] = None
) -> None:
    ...
    if PrettyTable:
        ...
        print_result = {}
        for class_name, iou, acc in zip(
            class_names, ret_metrics_class["IoU"], ret_metrics_class["Acc"]
        ):
