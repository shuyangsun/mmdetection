...
class YouTubeVISMetric(BaseVideoMetric):
    ...
    def __init__(
        ...
        for metric in self.metrics:
            if metric not in allowed_metrics:
                raise KeyError(f"metric should be 'youtube_vis_ap', but got {metric}.")
    ...
    def process_video(self, data_samples):
        ...
        for frame_id in range(video_length):
            ...
            result["masks"] = encode_mask_results(pred["masks"].detach().cpu().numpy())
    ...
    def _format_one_video_preds(self, pred_dicts: Tuple[dict]) -> List:
        ...
        for frame_id, (masks, scores, labels, ids) in enumerate(
            zip(preds["masks"], preds["scores"], preds["labels"], preds["instances_id"])
        ):
        ...
        for id, id_data in collect_data.items():
            ...
            output["category_id"] = (
                np.bincount(np.array(id_data["category_ids"])).argmax().item() + 1
            )
            output["segmentations"] = []
            for frame_id in range(inds[-1] - inds[-2]):
                if frame_id in id_data["segmentations"]:
                    output["segmentations"].append(id_data["segmentations"][frame_id])
                else:
