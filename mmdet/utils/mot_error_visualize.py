...
def _cv2_show_wrong_tracks(
    ...
    assert ids.ndim == 1, f" ids ndim should be 1, but its ndim is {ids.ndim}."
    assert (
        error_types.ndim == 1
    ), f" error_types ndim should be 1, but its ndim is {error_types.ndim}."
    assert (
        bboxes.shape[0] == ids.shape[0]
    ...
    bbox_colors = [
        [int(255 * _c) for _c in bbox_color][::-1] for bbox_color in bbox_colors
    ]
    ...
    for bbox, error_type, id in zip(bboxes, error_types, ids):
        ...
        img[y1 + text_height : y1 + text_height * 2, x1 : x1 + width, :] = bbox_color
        cv2.putText(
            img,
            str(id),
            (x1, y1 + text_height * 2 - 2),
            cv2.FONT_HERSHEY_COMPLEX,
            font_scale,
            color=(0, 0, 0),
        )
...
def _plt_show_wrong_tracks(
    ...
    plt.autoscale(False)
    plt.subplots_adjust(top=1, bottom=0, right=1, left=0, hspace=None, wspace=None)
    plt.margins(0, 0)
    ...
    for bbox, error_type, id in zip(bboxes, error_types, ids):
        ...
        plt.text(left_top[0], left_top[1] + text_height + 2, text, fontsize=font_scale)
        ...
        plt.gca().add_patch(
            Rectangle(
                (left_top[0], left_top[1] + text_height + 1),
                width,
                text_height,
                thickness,
                edgecolor=bbox_colors[error_type],
        ...
        plt.text(
            left_top[0], left_top[1] + 2 * (text_height + 1), text, fontsize=font_scale
        )
