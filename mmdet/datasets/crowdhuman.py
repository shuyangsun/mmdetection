...
class CrowdHumanDataset(BaseDetDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        if not self.extra_ann_exist and get_rank() == 0:
            ...
            print_log(f"\nsave extra_ann_file in {self.data_root}", level=logging.INFO)
    ...
    def parse_data_info(self, raw_data_info: dict) -> Union[dict, List[dict]]:
        ...
        """
        data_info = {}
        img_path = osp.join(self.data_prefix["img"], f"{raw_data_info['ID']}.jpg")
        ...
        for i, ann in enumerate(raw_data_info["gtboxes"]):
            ...
            if ann["tag"] not in self.metainfo["classes"]:
                ...
                instance["bbox_label"] = self.metainfo["classes"].index(ann["tag"])
            ...
            instance["fbox"] = bbox
            hbox = ann["hbox"]
            instance["hbox"] = [hbox[0], hbox[1], hbox[0] + hbox[2], hbox[1] + hbox[3]]
            vbox = ann["vbox"]
            instance["vbox"] = [vbox[0], vbox[1], vbox[0] + vbox[2], vbox[1] + vbox[3]]
