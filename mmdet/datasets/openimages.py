...
class OpenImagesDataset(BaseDetDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        """
        classes_names, label_id_mapping = self._parse_label_file(self.label_file)
        self._metainfo["classes"] = classes_names
        self.label_id_mapping = label_id_mapping

        if self.image_level_ann_file is not None:
            img_level_anns = self._parse_img_level_ann(self.image_level_ann_file)
        else:
            img_level_anns = None
        ...
        with get_local_path(
            ...
            with open(local_path, "r") as f:
                ...
                for i, line in enumerate(reader):
                    ...
                    last_img_path = osp.join(
                        self.data_prefix["img"], f"{last_img_id}.jpg"
                    )
        ...
        for i, meta in enumerate(img_metas):
            ...
            if img_level_anns is not None:
                ...
                data_list[i]["confidences"] = np.array(confidences, dtype=np.float32)
        return data_list
    ...
    def _parse_img_level_ann(self, img_level_ann_file: str) -> Dict[str, List[dict]]:
        ...
        with get_local_path(
            img_level_ann_file, backend_args=self.backend_args
        ) as local_path:
            with open(local_path, "r") as f:
                reader = csv.reader(f)
                for i, line in enumerate(reader):
                    ...
                    item_lists[img_id].append(
                        dict(
                            image_level_label=int(self.label_id_mapping[line[2]]),
    ...
    def _get_relation_matrix(self, hierarchy_file: str) -> np.ndarray:
        ...
        relation_matrix = self._convert_hierarchy_tree(hierarchy, relation_matrix)
    ...
    def _join_prefix(self):
        ...
        if self.image_level_ann_file and not is_abs(self.image_level_ann_file):
            self.image_level_ann_file = osp.join(
                self.data_root, self.image_level_ann_file
            )
...
class OpenImagesChallengeDataset(OpenImagesDataset):
    ...
    def _parse_img_level_ann(self, image_level_ann_file):
        ...
        item_lists = defaultdict(list)
        with get_local_path(
            image_level_ann_file, backend_args=self.backend_args
        ) as local_path:
            with open(local_path, "r") as f:
                reader = csv.reader(f)
                i = -1
                for line in reader:
                    ...
                    if i == 0:
                        ...
                        label_id = line[1]
                        assert label_id in self.label_id_mapping
                        image_level_label = int(self.label_id_mapping[label_id])
    ...
    def _get_relation_matrix(self, hierarchy_file: str) -> np.ndarray:
        ...
        """
        with get_local_path(
            hierarchy_file, backend_args=self.backend_args
        ) as local_path:
