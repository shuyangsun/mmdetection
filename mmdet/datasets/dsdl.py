...
class DSDLDetDataset(BaseDetDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if kwargs.get("data_root"):
            kwargs["ann_file"] = os.path.join(kwargs["data_root"], kwargs["ann_file"])
        ...
        if with_imagelevel_label:
            ...
            assert (
                "image_level_labels" in specific_key_path.keys()
    ...
    def filter_data(self) -> List[dict]:
        ...
        min_size = (
            self.filter_cfg.get("min_size", 0) if self.filter_cfg is not None else 0
        )
