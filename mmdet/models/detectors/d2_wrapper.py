...
def _to_cfgnode_list(
    ...
    for key, value in cfg.items():
        ...
        if isinstance(value, ConfigDict) or isinstance(value, dict):
            config_list, fater_name = _to_cfgnode_list(value, config_list, name)
        else:
...
class Detectron2Wrapper(BaseDetector):
    ...
    def __init__(
        self, detector: ConfigType, bgr_to_rgb: bool = False, rgb_to_bgr: bool = False
    ) -> None:
    ...
    def _convert_to_d2_inputs(
        self, batch_inputs: Tensor, batch_data_samples: SampleList, training=True
    ) -> list:
        ...
        for image, data_samples in zip(batch_inputs, batch_data_samples):
            ...
            if training:
                ...
                d2_instances = filter_empty_instances(d2_instances.to("cpu")).to(device)
