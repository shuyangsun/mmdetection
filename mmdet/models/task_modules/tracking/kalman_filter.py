...
class KalmanFilter:
    ...
    def __init__(self, center_only: bool = False, use_nsa: bool = False):
        if not HAS_SCIPY:
            raise RuntimeError(
                "sscikit-learn is not installed,\
    ...
    def predict(
        self, mean: np.array, covariance: np.array
    ...
    def project(
        self, mean: np.array, covariance: np.array, bbox_score: float = 0.0
    ) -> Tuple[np.array, np.array]:
    ...
    def update(
        ...
        Returns:
             ...
             distribution.
        """
        projected_mean, projected_cov = self.project(mean, covariance, bbox_score)
    ...
    def track(self, tracks: dict, bboxes: torch.Tensor) -> Tuple[dict, np.array]:
        """Track forward.
        ...
        for id, track in tracks.items():
            track.mean, track.covariance = self.predict(track.mean, track.covariance)
            gating_distance = self.gating_distance(
                track.mean, track.covariance, bboxes.cpu().numpy(), self.center_only
            )
