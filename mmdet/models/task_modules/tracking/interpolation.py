...
class InterpolateTracklets:
    ...
    def _interpolate_track(
        self, track: np.ndarray, track_id: int, max_num_frames: int = 20
    ) -> np.ndarray:
        ...
        assert (
            track[:, 1] == track_id
        ).all(), "The track id should not changed when interpolate a track."
        ...
        for i in np.where(np.diff(frame_ids) > 1)[0]:
            ...
            if 1 < num_disconnected_frames < max_num_frames:
                ...
                for j in range(1, num_disconnected_frames):
                    cur_bbox = (
                        j / (num_disconnected_frames) * (right_bbox - left_bbox)
                        + left_bbox
                    )
    ...
    def gaussian_smoothed_interpolation(
        self, track: np.ndarray, smooth_tau: int = 10
    ) -> np.ndarray:
        ...
        """
        len_scale = np.clip(
            smooth_tau * np.log(smooth_tau**3 / len(track)),
            smooth_tau**-1,
            smooth_tau**2,
        )
