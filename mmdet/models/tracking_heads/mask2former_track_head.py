...
class Mask2FormerTrackHead(MaskFormerHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            pixel_decoder.encoder.layer_cfg.self_attn_cfg.num_levels
        ...
        self.transformer_decoder = Mask2FormerTransformerDecoder(**transformer_decoder)
    ...
    def preprocess_gt(self, batch_gt_instances: InstanceList) -> InstanceList:
        ...
        for batch_idx in range(batch_size):
            pair_gt_insatences = batch_gt_instances[
                batch_idx * self.num_frames : batch_idx * self.num_frames
                + self.num_frames
            ]

            assert (
                len(pair_gt_insatences) > 1
            ), f"mask2former for vis need multi frames to train, \
            ...
            all_ins_id = torch.cat(
                [gt_instances.instances_ids for gt_instances in pair_gt_insatences]
            )
            ...
            gt_ids_per_video = torch.full(
                (num_instances, self.num_frames), -1, dtype=torch.long, device=_device
            )
            gt_labels_per_video = torch.full(
                (num_instances,), -1, dtype=torch.long, device=_device
            )
            ...
            for frame_id in range(self.num_frames):
                ...
                for i, id in enumerate(ins_ids):
                    gt_masks_per_video[
                        map_ins_id[id], frame_id, :, :
                    ] = cur_frame_gts.masks[i]
                    gt_ids_per_video[
                        map_ins_id[id], frame_id
                    ] = cur_frame_gts.instances_ids[i]
                    gt_labels_per_video[map_ins_id[id]] = cur_frame_gts.labels[i]
    ...
    def _loss_by_feat_single(
        ...
        with torch.no_grad():
            points_coords = get_uncertain_point_coords_with_randomness(
                mask_preds.flatten(0, 1).unsqueeze(1),
                None,
                self.num_points,
            ...
            mask_point_targets = point_sample(
                mask_targets.flatten(0, 1).unsqueeze(1).float(), points_coords
            ).squeeze(1)
        # shape (num_total_gts * num_frames, num_points)
        mask_point_preds = point_sample(
            mask_preds.flatten(0, 1).unsqueeze(1), points_coords
        ).squeeze(1)
    ...
    def forward(
        ...
        for i in range(self.num_transformer_feat_level):
            ...
            # (batch_size, t*h*w, c)
            decoder_input = (
                decoder_input.view(batch_size, t, c, hw)
                .permute(0, 1, 3, 2)
                .flatten(1, 2)
            )
            # shape (batch_size, c, h, w) -> (h*w, batch_size, c)
            mask = decoder_input.new_zeros(
                (batch_size, t) + multi_scale_memorys[i].shape[-2:], dtype=torch.bool
            )
            decoder_positional_encoding = self.decoder_positional_encoding(mask)
            decoder_positional_encoding = (
                decoder_positional_encoding.flatten(3).permute(0, 1, 3, 2).flatten(1, 2)
            )
    ...
    def predict(
        self, x: Tuple[Tensor], data_samples: TrackDataSample, rescale: bool = True
    ) -> InstanceList:
        ...
        batch_img_metas = [
            data_samples[img_idx].metainfo for img_idx in range(len(data_samples))
        ]
        all_cls_scores, all_mask_preds = self(x, data_samples)
    ...
    def predict_by_feat(
        ...
        if len(mask_cls_results) > 0:
            ...
            mask_pred_results = mask_pred_results[:, :, : img_shape[0], : img_shape[1]]
            if rescale:
