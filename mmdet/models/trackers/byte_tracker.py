...
class ByteTracker(BaseTracker):
    ...
    def track(self, data_sample: DetDataSample, **kwargs) -> InstanceData:
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            second_det_inds = (~first_det_inds) & (scores > self.obj_score_thrs["low"])
            ...
            first_det_ids[valid] = torch.tensor(self.confirmed_ids)[
                first_match_det_inds[valid]
            ].to(labels)
        ...
        self.update(
            ids=ids, bboxes=bboxes, scores=scores, labels=labels, frame_ids=frame_id
        )
