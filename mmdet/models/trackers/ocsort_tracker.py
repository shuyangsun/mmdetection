...
class OCSORTTracker(SORTTracker):
    ...
    def init_track(self, id: int, obj: Tuple[torch.Tensor]):
        ...
        self.tracks[id].velocity = torch.tensor((-1, -1)).to(
            obj[bbox_id].device
        )  # placeholder
    ...
    def vel_direction_batch(self, bboxes1: torch.Tensor, bboxes2: torch.Tensor):
        """Estimate the direction vector given two batches of boxes."""
        cx1, cy1 = (bboxes1[:, 0] + bboxes1[:, 2]) / 2.0, (
            bboxes1[:, 1] + bboxes1[:, 3]
        ) / 2.0
        cx2, cy2 = (bboxes2[:, 0] + bboxes2[:, 2]) / 2.0, (
            bboxes2[:, 1] + bboxes2[:, 3]
        ) / 2.0
        speed_diff_y = cy2[None, :] - cy1[:, None]
        speed_diff_x = cx2[None, :] - cx1[:, None]
        speed = torch.cat((speed_diff_y[..., None], speed_diff_x[..., None]), dim=-1)
        norm = torch.sqrt((speed[:, :, 0]) ** 2 + (speed[:, :, 1]) ** 2) + 1e-6
        speed[:, :, 0] /= norm
        speed[:, :, 1] /= norm
        return speed
    ...
    def ocm_assign_ids(
        ...
        track_labels = torch.tensor([self.tracks[id]["labels"][-1] for id in ids]).to(
            det_bboxes.device
        )
        ...
        if len(ids) > 0 and len(det_bboxes) > 0:
            track_velocities = torch.stack([self.tracks[id].velocity for id in ids]).to(
                det_bboxes.device
            )
            ...
            vel_to_match = self.vel_direction_batch(k_step_observations, det_bboxes)
            ...
            norm_angle = (angle - np.pi / 2.0) / np.pi  # [-0.5, 0.5]
    ...
    def online_smooth(self, track: Dict, obj: torch.Tensor):
        ...
        for bbox in track.obs[::-1]:
            if bbox is None:
                ...
                break
        bbox_shift_per_step = (new_match_bbox - last_match_bbox) / (unmatch_len + 1)
    ...
    def track(self, data_sample: DetDataSample, **kwargs) -> InstanceData:
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            ids = torch.arange(self.num_tracks, self.num_tracks + num_new_tracks).to(
                labels
            )
            ...
            # 0. init
            ids = torch.full(
                (bboxes.size(0),), -1, dtype=labels.dtype, device=labels.device
            )
            ...
            det_ids[valid] = torch.tensor(self.confirmed_ids)[
                raw_match_det_inds[valid]
            ].to(labels)
            ...
            if len(unmatched_track_inds) > 0:
                ...
                remain_det_ids = torch.full(
                    (unmatch_det_bboxes.size(0),),
                    -1,
                    dtype=labels.dtype,
                ...
                match_det_ids = torch.cat((match_det_ids, ocr_match_det_ids), dim=0)
        ...
        self.update(
            ids=ids, bboxes=bboxes, labels=labels, scores=scores, frame_ids=frame_id
        )
