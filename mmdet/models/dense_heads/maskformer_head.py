...
class MaskFormerHead(AnchorFreeHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.transformer_decoder = DetrTransformerDecoder(**transformer_decoder)
    ...
    def preprocess_gt(
        ...
        gt_masks_list = [gt_instances["masks"] for gt_instances in batch_gt_instances]
        gt_semantic_segs = [
        ...
        batch_gt_instances = [
            InstanceData(labels=label, masks=mask) for label, mask in zip(labels, masks)
        ]
    ...
    def _get_targets_single(
        ...
        if gt_masks.shape[0] > 0:
            gt_masks_downsampled = (
                F.interpolate(
                    gt_masks.unsqueeze(1).float(), target_shape, mode="nearest"
                )
                .squeeze(1)
                .long()
            )
    ...
    def forward(
        ...
        padding_mask = (
            F.interpolate(
                padding_mask.unsqueeze(1), size=x[-1].shape[-2:], mode="nearest"
            )
            .to(torch.bool)
            .squeeze(1)
        )
