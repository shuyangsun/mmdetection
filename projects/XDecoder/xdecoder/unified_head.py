...
class XDecoderUnifiedhead(nn.Module):
        ...
        if self.task != "caption":
            ...
            for data_samples in batch_data_samples:
                if isinstance(data_samples.text, str):
                    ...
                    raise TypeError("Type pf data_sample.text must be sequence or str")
            ...
            if all_text_prompts != self._all_text_prompts or self._force_not_use_cache:
            ...
            if not hasattr(self, "start_token"):
                self.start_token = self.predictor.lang_encoder.get_sot_token(
                    device=device
                )
    ...
    def predict(self, features, batch_data_samples):
        ...
        return self.post_process(
            predictions, batch_data_samples, all_text_prompts, num_thing_class
        )

    def post_process(
        self, predictions, batch_data_samples, all_text_prompts, num_thing_class
    ):
        batch_img_metas = [data_samples.metainfo for data_samples in batch_data_samples]
        batch_input_shape = batch_data_samples[0].metainfo["batch_input_shape"]

        if self.task == "caption":
            for text, data_samples in zip(
                predictions["pred_caption"], batch_data_samples
            ):
            ...
            if "pred_instances" in batch_data_samples[0]:
                for img_metas, data_samples in zip(batch_img_metas, batch_data_samples):
                    ...
                    text_prompts = list(filter(lambda x: len(x) > 0, original_caption))
                    ...
                    pred_instances = retry_if_cuda_oom(self._instance_inference)(
                        mask_cls_result, mask_pred_result, text_prompts
                    )
            ...
            for mask_cls_result, mask_pred_result, img_metas, data_samples in zip(
                mask_cls_results, mask_pred_results, batch_img_metas, batch_data_samples
            ):
                height = img_metas["ori_shape"][0]
                width = img_metas["ori_shape"][1]
                ...
                if self.task == "semseg":
                    ...
                    pred_panoptic_seg = retry_if_cuda_oom(self._panoptic_inference)(
                        mask_cls_result,
                        mask_pred_result,
                        all_text_prompts,
                        num_thing_class,
            ...
            mask_cls_results = predictions["pred_logits"]
            results_ = zip(
                mask_pred_results, mask_cls_results, batch_img_metas, batch_data_samples
            )
            for mask_pred_result, mask_cls_result, img_metas, data_samples in results_:
                if is_lower_torch_version():
    ...
    def _instance_inference(self, mask_cls, mask_pred, text_prompts):
        ...
        if self.task in ["ref-seg", "caption"]:
            ...
            labels = (
                torch.arange(num_class, device=scores.device)
                .unsqueeze(0)
        ...
        mask_scores_per_image = (mask_pred.flatten(1) * result.masks.flatten(1)).sum(
            1
    ...
    def _panoptic_inference(
        self, mask_cls, mask_pred, all_text_prompts, num_thing_class
    ):
        scores, labels = F.softmax(mask_cls, dim=-1).max(-1)
        mask_pred = mask_pred.sigmoid()

        keep = labels.ne(len(all_text_prompts)) & (scores > self.test_cfg.mask_thr)
            ...
            for k in range(cur_classes.shape[0]):
                ...
                if mask_area > 0 and original_area > 0 and mask.sum().item() > 0:
                    ...
                    if not isthing:
                        ...
                        panoptic_seg[mask] = (
                            int(pred_class) + instance_id * INSTANCE_OFFSET
                        )
                        instance_id += 1
