...
class LanguageEncoder(nn.Module):
    def __init__(
        ...
        self.lang_encoder = Transformer(
            max_token_num, self.tokenizer.vocab_size, dim_lang
        )
    ...
    def forward_language(
        self, texts, norm=False, with_token_embed=True, with_cls_embed=True
    ):
        ...
        class_embed = None
        if with_cls_embed:
            class_embed = hidden_x[
                torch.arange(hidden_x.size(0)), texts[0].argmax(dim=-1)
            ]
