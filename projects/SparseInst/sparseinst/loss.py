...
class SparseInstCriterion(nn.Module):
    ...
    def loss_classification(self, outputs, batch_gt_instances, indices, num_instances):
    ...
    def loss_masks_with_iou_objectness(
        self, outputs, batch_gt_instances, indices, num_instances
    ):
        ...
        with torch.no_grad():
            target_masks = torch.cat(
                [
                    gt.masks.to_tensor(dtype=src_masks.dtype, device=src_masks.device)
    ...
    def forward(
        self, outputs, batch_gt_instances, batch_img_metas, batch_gt_instances_ignore
    ):
