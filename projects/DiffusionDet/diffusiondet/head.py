...
class DynamicDiffusionDetHead(nn.Module):
    def __init__(
        ...
        criterion=dict(
            ...
            assigner=dict(
                ...
                match_costs=[
                    dict(type="FocalLossCost", alpha=2.0, gamma=0.25, weight=2.0),
        ...
        **kwargs,
        ...
        assert isinstance(timesteps, int), (
            "The type of `timesteps` should " f"be int but got {type(timesteps)}"
        )
        ...
        default_pooler_resolution = roi_extractor["roi_layer"].get("output_size")
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: SampleList) -> dict:
        ...
        output = {"pred_logits": pred_logits[-1], "pred_boxes": pred_bboxes[-1]}
        if self.deep_supervision:
            output["aux_outputs"] = [
    ...
    def prepare_diffusion(self, gt_boxes, image_size):
        ...
        if num_gt < self.num_proposals:
            ...
            box_placeholder = (
                torch.randn(self.num_proposals - num_gt, 4, device=device) / 6.0 + 0.5
            )
    ...
    def q_sample(self, x_start, time, noise=None):
        ...
        sqrt_alphas_cumprod_t = extract(self.sqrt_alphas_cumprod, time, x_start_shape)
    ...
    def predict_by_feat(
        ...
        for time, time_next in time_pairs:
            ...
            x_start = (x_start * 2 - 1.0) * self.snr_scale
            x_start = torch.clamp(x_start, min=-1 * self.snr_scale, max=self.snr_scale)
            ...
            for idx in range(batch_size):
                ...
                if self.box_renewal:  # filter
                    ...
                    if num_remain < self.num_proposals:
                        noise_bboxes = torch.cat(
                            (
                                noise_bboxes,
                                torch.randn(
                                    self.num_proposals - num_remain, 4, device=device
                                ),
                            ),
                            dim=0,
                        )
                    ...
                    noise_bboxes = torch.clamp(
                        noise_bboxes, min=-1 * self.snr_scale, max=self.snr_scale
                    )
        ...
        if self.use_ensemble and self.sampling_timesteps > 1:
            ...
            results_list = []
            for idx in range(batch_size):
                ensemble_score_per_img = [ensemble_score[i][idx] for i in range(steps)]
                ensemble_label_per_img = [ensemble_label[i][idx] for i in range(steps)]
                ensemble_coord_per_img = [ensemble_coord[i][idx] for i in range(steps)]
                ...
                if self.use_nms:
                    det_bboxes, keep_idxs = batched_nms(
                        box_pred_per_image, scores_per_image, labels_per_image, cfg.nms
                    )
    ...
    def do_results_post_process(results_list, cfg, batch_img_metas=None):
        ...
        for results, img_meta in zip(results_list, batch_img_metas):
            ...
            results.bboxes[:, 0::2] = results.bboxes[:, 0::2].clamp(min=0, max=w)
            results.bboxes[:, 1::2] = results.bboxes[:, 1::2].clamp(min=0, max=h)
    ...
    def prepare_testing_targets(self, batch_img_metas, device):
        ...
        for img_meta in batch_img_metas:
            h, w = img_meta["img_shape"]
            image_size = torch.tensor([w, h, w, h], dtype=torch.float32, device=device)
            noise_bboxes_raw = torch.randn((self.num_proposals, 4), device=device)
            noise_bboxes = torch.clamp(
...
class SingleDiffusionDetHead(nn.Module):
    ...
    def forward(self, features, bboxes, pro_features, pooler, time_emb):
        ...
        if pro_features is None:
            pro_features = roi_features.view(N, num_boxes, self.feat_channels, -1).mean(
                -1
            )

        roi_features = roi_features.view(N * num_boxes, self.feat_channels, -1).permute(
            2, 0, 1
        )

        # self_att.
        pro_features = pro_features.view(N, num_boxes, self.feat_channels).permute(
            1, 0, 2
        )
