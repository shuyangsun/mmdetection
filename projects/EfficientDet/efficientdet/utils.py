...
def variance_scaling_trunc(tensor, gain=1.0):
    fan_in, _ = _calculate_fan_in_and_fan_out(tensor)
    gain /= max(1.0, fan_in)
    std = math.sqrt(gain) / 0.87962566103423978
    return trunc_normal_(tensor, 0.0, std)
...
class Conv2dSamePadding(nn.Conv2d):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        extra_w = (
            (math.ceil(img_w / self.stride[1]) - 1) * self.stride[1] - img_w + kernel_w
        )
        extra_h = (
            (math.ceil(img_h / self.stride[0]) - 1) * self.stride[0] - img_h + kernel_h
        )
