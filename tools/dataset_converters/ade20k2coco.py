        ...
        for filename in filenames:
            ...
            for semantic_cat_id in np.unique(semantic_cat_ids):
                if semantic_cat_id == 255:
                    continue
                if (
                    categories_dict[old_2_new_mapping[int(semantic_cat_id)]]["isthing"]
                    == 1
                ):
                    continue
            ...
            for thing_id in np.unique(instance_ins_ids):
                ...
                segment_id, color = id_generator.get_id_and_color(instance_cat_id[0])
                pan_seg[mask] = color
...
def main():
    ...
    if args.task == "panoptic":
        ...
        if os.path.exists(annotation_train_path) or os.path.exists(annotation_val_path):
