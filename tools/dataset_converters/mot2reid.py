...
def main():
    ...
    for video_name in tqdm(video_names):
        ...
        for gt in gts:
            ...
            if frame_id != last_frame_id:
                ...
                raw_img = mmcv.imread(f"{video_folder}/{raw_img_folder}/{raw_img_name}")
                last_frame_id = frame_id
            xyxy = np.asarray([ltwh[0], ltwh[1], ltwh[0] + ltwh[2], ltwh[1] + ltwh[3]])
    ...
    for reid_img_folder_name in reid_img_folder_names[num_train_ids:]:
        ...
        val_label += 1
    with open(
        osp.join(reid_meta_folder, f"train_{int(100 * (1 - args.val_split))}.txt"), "w"
    ) as f:
        f.writelines(reid_train_list)
    with open(
        osp.join(reid_meta_folder, f"val_{int(100 * args.val_split)}.txt"), "w"
    ) as f:
