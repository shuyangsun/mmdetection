...
def cvt_annotations(devkit_path, years, split, out_file):
    ...
    for year in years:
        ...
        part_annotations = track_progress(parse_xml, list(zip(xml_paths, img_paths)))
...
def main():
    ...
    for year in years:
        ...
        for split in ["train", "val", "trainval"]:
            ...
            cvt_annotations(
                devkit_path, year, split, osp.join(out_dir, dataset_name + out_fmt)
            )
        if not isinstance(year, list):
            dataset_name = prefix + "_test"
            print(f"processing {dataset_name} ...")
            cvt_annotations(
                devkit_path, year, "test", osp.join(out_dir, dataset_name + out_fmt)
            )
    print("Done!")
