...
def parse_args():
    parser = argparse.ArgumentParser(description="YouTube-VIS to COCO Video format")
    parser.add_argument(
        "-i",
        "--input",
...
def convert_vis(ann_dir, save_dir, dataset_version, mode="train"):
    ...
    if dataset_version == "2019":
        ...
        official_anns = mmengine.load(osp.join(ann_dir, mode, "instances.json"))
    ...
    mmengine.dump(VIS, osp.join(save_dir, f"youtube_vis_{dataset_version}_{mode}.json"))
