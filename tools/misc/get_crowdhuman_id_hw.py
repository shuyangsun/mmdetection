...
def parse_args():
    ...
    parser.add_argument(
        "--nproc", default=10, type=int, help="Processes used for get image metas"
    )
...
def main():
    ...
    img_prefix = osp.join(
        dataloader_cfg.dataset.data_root, dataloader_cfg.dataset.data_prefix["img"]
    )

    # load image metas
    print_log(f"loading CrowdHuman {dataset} annotation...", level=logging.INFO)
