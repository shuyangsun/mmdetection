...
def main():
    ...
    metrics_types = (
        ...
        else cfg.test_evaluator.metric
    )
        ...
        if rank == 0:
            ...
            for metric in search_metrics:
                res = runner.message_hub.get_scalar(
                    "test/motchallenge-metric/" + metric
                ).current()
                if isinstance(res, float):
                    _records.append(f"{res:.3f}")
                else:
                    _records.append(f"{res}")
            print_log(f"-------------- {case}: {_records} --------------", logger)
