...
def parse_args():
    ...
    parser.add_argument("--result-dir", help="directory of the inference result")
    parser.add_argument(
        "--output-dir", help="directory where painted images or videos will be saved"
    )
    parser.add_argument(
        "--show", action="store_true", help="whether to show the results on the fly"
    )
    parser.add_argument("--fps", type=int, default=3, help="FPS of the output video")
    parser.add_argument(
        "--backend",
        type=str,
        choices=["cv2", "plt"],
...
def compare_res_gts(results_dir: str, dataset: Dataset, video_name: str):
    ...
    if "half-train" in dataset.ann_file:
        gt_file = osp.join(
            dataset.data_prefix["img_path"], f"{video_name}/gt/gt_half-train.txt"
        )
        gt = mm.io.loadtxt(gt_file)
        gt.index = gt.index.set_levels(pd.factorize(gt.index.levels[0])[0] + 1, level=0)
        ...
        gt_file = osp.join(dataset.data_prefix["img_path"], f"{video_name}/gt/gt.txt")
        gt = mm.io.loadtxt(gt_file)
        gt.index = gt.index.set_levels(pd.factorize(gt.index.levels[0])[0] + 1, level=0)
    res_file = osp.join(results_dir, f"{video_name}.txt")
    res = mm.io.loadtxt(res_file)
    ini_file = osp.join(dataset.data_prefix["img_path"], f"{video_name}/seqinfo.ini")
...
def main():
    ...
    for video_name in video_names:
        ...
        frames_id_list = sorted(list(set(acc.mot_events.index.get_level_values(0))))
        for frame_id in frames_id_list:
            ...
            imshow_mot_errors(
                ...
                error_types,
                show=args.show,
                out_file=osp.join(args.out_dir, f"{video_name}/{frame_id:06d}.jpg")
                if args.out_dir
                else None,
        ...
        print_log(
            f"Done! Visualization images are saved in " f"'{args.out_dir}/{video_name}'"
        )
