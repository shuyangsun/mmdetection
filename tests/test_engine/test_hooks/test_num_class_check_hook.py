...
class TestNumClassCheckHook(TestCase):
    def setUp(self):
        ...
        self.none_classmeta_dataset = BaseDataset(metainfo=metainfo, lazy_init=True)
        metainfo = dict(classes="class_name")
        self.str_classmeta_dataset = BaseDataset(metainfo=metainfo, lazy_init=True)
        metainfo = dict(classes=("bus", "car"))
        self.normal_classmeta_dataset = BaseDataset(metainfo=metainfo, lazy_init=True)

        # Setup valid model
        valid_model = nn.Module()
        valid_model.add_module("backbone", VGG(depth=11))
        fused_semantic_head = FusedSemanticHead(
            num_ins=1, fusion_level=0, num_convs=1, in_channels=1, conv_out_channels=1
        )
