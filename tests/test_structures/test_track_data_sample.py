...
class TestDetDataSample(TestCase):
    def test_init(self):
        ...
        assert (
            ...
            and "ref_frames_inds" in track_data_sample.metainfo
        )
    ...
    def test_setter(self):
        det_data_sample_1 = DetDataSample(metainfo=dict(scale_factor=(1.5, 1.5)))
        det_data_sample_2 = DetDataSample(metainfo=dict(scale_factor=(2.0, 2.0)))
        track_data_sample = TrackDataSample(
            metainfo=dict(key_frames_inds=[0], ref_frames_inds=[1])
        )
        track_data_sample.video_data_samples = [det_data_sample_1, det_data_sample_2]

        assert track_data_sample.get_key_frames()[0].scale_factor == (1.5, 1.5)
        assert track_data_sample.get_ref_frames()[0].scale_factor == (2.0, 2.0)
