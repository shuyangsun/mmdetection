...
class TestDetDataSample(TestCase):
    ...
    def test_setter(self):
        ...
        gt_instances_data = dict(
            bboxes=torch.rand(4, 4), labels=torch.rand(4), masks=np.random.rand(4, 2, 2)
        )
        gt_instances = InstanceData(**gt_instances_data)
        det_data_sample.gt_instances = gt_instances
        assert "gt_instances" in det_data_sample
        assert _equal(det_data_sample.gt_instances.bboxes, gt_instances_data["bboxes"])
        assert _equal(det_data_sample.gt_instances.labels, gt_instances_data["labels"])
        assert _equal(det_data_sample.gt_instances.masks, gt_instances_data["masks"])

        # test pred_instances
        pred_instances_data = dict(
            bboxes=torch.rand(2, 4), labels=torch.rand(2), masks=np.random.rand(2, 2, 2)
        )
        ...
        assert _equal(
            det_data_sample.pred_instances.bboxes, pred_instances_data["bboxes"]
        )
        assert _equal(
            det_data_sample.pred_instances.labels, pred_instances_data["labels"]
        )
        assert _equal(
        ...
        assert "pred_instances" in det_data_sample
        assert _equal(
            det_data_sample.pred_instances.bboxes, pred_track_instances_data["bboxes"]
        )
        assert _equal(
            det_data_sample.pred_instances.labels, pred_track_instances_data["labels"]
        )
        assert _equal(
            det_data_sample.pred_instances.masks, pred_track_instances_data["masks"]
        )
        ...
        assert _equal(det_data_sample.proposals.bboxes, proposals_data["bboxes"])
        assert _equal(det_data_sample.proposals.labels, proposals_data["labels"])

        # test ignored_instances
        ignored_instances_data = dict(bboxes=torch.rand(4, 4), labels=torch.rand(4))
        ...
        assert _equal(
            det_data_sample.ignored_instances.bboxes, ignored_instances_data["bboxes"]
        )
        assert _equal(
            det_data_sample.ignored_instances.labels, ignored_instances_data["labels"]
        )
        ...
        assert _equal(
            det_data_sample.pred_segm_seg.segm_seg, pred_segm_seg_data["segm_seg"]
        )
