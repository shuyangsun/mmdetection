...
class TestCocoMetric(TestCase):
    ...
    def test_evaluate(self):
        ...
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test.bbox.json")))
        ...
        self.assertDictEqual(eval_results, target)
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test.bbox.json")))
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test.segm.json")))

        # test invalid custom metric_items
        with self.assertRaisesRegex(KeyError, 'metric item "invalid" is not supported'):
            coco_metric = CocoMetric(ann_file=fake_json_file, metric_items=["invalid"])
            coco_metric.dataset_meta = dict(classes=["car", "bicycle"])
            coco_metric.process(
                {}, [dict(pred_instances=dummy_pred, img_id=0, ori_shape=(640, 640))]
            )
        ...
        coco_metric = CocoMetric(ann_file=fake_json_file, metric_items=["mAP_m"])
        coco_metric.dataset_meta = dict(classes=["car", "bicycle"])
        coco_metric.process(
            {}, [dict(pred_instances=dummy_pred, img_id=0, ori_shape=(640, 640))]
        )
    ...
    def test_classwise_evaluate(self):
        ...
        # test single coco dataset evaluation
        coco_metric = CocoMetric(ann_file=fake_json_file, metric="bbox", classwise=True)
    ...
    def test_fast_eval_recall(self):
        ...
        coco_metric = CocoMetric(ann_file=fake_json_file, metric="proposal_fast")
        coco_metric.dataset_meta = dict(classes=["car", "bicycle"])
        coco_metric.process(
            {}, [dict(pred_instances=dummy_pred, img_id=0, ori_shape=(640, 640))]
        )
        ...
        coco_metric = CocoMetric(
            ann_file=fake_json_file, metric="proposal_fast", proposal_nums=(2, 4)
        )
        coco_metric.dataset_meta = dict(classes=["car", "bicycle"])
        coco_metric.process(
            {}, [dict(pred_instances=dummy_pred, img_id=0, ori_shape=(640, 640))]
    ...
    def test_evaluate_without_json(self):
        ...
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test.bbox.json")))
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test.segm.json")))
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test.gt.json")))
