...
class TestLVISMetric(unittest.TestCase):
    def _create_dummy_lvis_json(self, json_name):
        ...
        categories = [
            {"id": 1, "name": "aerosol_can", "frequency": "c", "image_count": 64},
            {"id": 2, "name": "air_conditioner", "frequency": "f", "image_count": 364},
        ]
    ...
    def test_evaluate(self):
        ...
        lvis_metric.dataset_meta = dict(classes=["aerosol_can", "air_conditioner"])
        lvis_metric.process(
        ...
        with self.assertRaisesRegex(
            ...
            lvis_metric = LVISMetric(ann_file=fake_json_file, metric=["invalid"])
    ...
    def test_classwise_evaluate(self):
        ...
        # test single lvis dataset evaluation
        lvis_metric = LVISMetric(ann_file=fake_json_file, metric="bbox", classwise=True)
        lvis_metric.dataset_meta = dict(classes=["aerosol_can", "air_conditioner"])
        lvis_metric.process(
            {}, [dict(pred_instances=dummy_pred, img_id=0, ori_shape=(640, 640))]
        )
    ...
    def test_fast_eval_recall(self):
        ...
        lvis_metric = LVISMetric(ann_file=fake_json_file, metric="proposal_fast")
        lvis_metric.dataset_meta = dict(classes=["aerosol_can", "air_conditioner"])
        lvis_metric.process(
            {}, [dict(pred_instances=dummy_pred, img_id=0, ori_shape=(640, 640))]
        )
