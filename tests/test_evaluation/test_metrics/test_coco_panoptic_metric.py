...
class TestCocoPanopticMetric(unittest.TestCase):
    def _create_panoptic_gt_annotations(self, ann_file, seg_map_dir):
        categories = [
            {"id": 0, "name": "person", "supercategory": "person", "isthing": 1},
            {"id": 1, "name": "cat", "supercategory": "cat", "isthing": 1},
            {"id": 2, "name": "dog", "supercategory": "dog", "isthing": 1},
            {"id": 3, "name": "wall", "supercategory": "wall", "isthing": 0},
        ]
        ...
        gt_bboxes = np.array(
            [[10, 10, 10, 40], [30, 10, 10, 40], [50, 10, 10, 5]], dtype=np.int64
        )
        for i in range(3):
            x, y, w, h = gt_bboxes[i]
        ...
        data_samples = [
            {
                ...
                "seg_map_path": osp.join(self.gt_seg_dir, "fake_name1.png"),
                "pred_panoptic_seg": {"sem_seg": torch.from_numpy(pred).unsqueeze(0)},
            }
        ]
    ...
    def setUp(self):
        ...
        self._create_panoptic_gt_annotations(self.gt_json_path, self.gt_seg_dir)
        self.dataset_meta = {
