...
class TestReIDDataset(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.dataset_triplet = ReIDDataset(
            pipeline=[],
            triplet_sampler=dict(num_ids=cls.num_ids, ins_per_id=cls.ins_per_id),
    ...
    def test_get_data_info(self):
        # id 0 has 21 objects
        img_id = 0
        data_list = [self.dataset.get_data_info(i) for i in range(len(self.dataset))]
        assert (
            len(
                [
                    data_info
        ...
        for i in range(len(self.dataset_triplet)):
            ...
            for idx in range(num - 1):
                if (idx + 1) % self.ins_per_id != 0:
                    assert results["gt_label"][idx] == results["gt_label"][idx + 1]
