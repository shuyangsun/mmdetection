...
class TestResize(unittest.TestCase):
    ...
    def test_resize(self):
        ...
        results = transform(copy.deepcopy(self.data_info1))
        self.assertTrue(
            (results["gt_bboxes"] == np.array([[0.0, 0.0, 168.0, 224.0]])).all()
        )
        ...
        self.assertTrue(
            (results["gt_bboxes"] == np.array([40.0, 11.25, 120.0, 33.75])).all()
        )
    ...
    def test_resize_use_box_type(self):
        ...
        results = transform(copy.deepcopy(data_info1))
        self.assertTrue(
            (results["gt_bboxes"].numpy() == np.array([[0, 0, 168, 224]])).all()
        )
        self.assertEqual(results["gt_masks"].height, 800)
        self.assertEqual(results["gt_masks"].width, 750)
        self.assertEqual(results["gt_seg_map"].shape[:2], (800, 750))
        ...
        self.assertTrue(
            (
                results["gt_bboxes"].numpy() == np.array([40.0, 11.25, 120.0, 33.75])
            ).all()
        )
...
class TestFixShapeResize(unittest.TestCase):
    ...
    def test_resize(self):
        ...
        # test clip_object_border = False
        transform = FixShapeResize(width=200, height=150, clip_object_border=False)
        results = transform(copy.deepcopy(self.data_info2))
        self.assertTrue((results["gt_bboxes"] == np.array([100, 75, 300, 225])).all())
    ...
    def test_resize_with_boxlist(self):
        ...
        results = transform(copy.deepcopy(data_info1))
        self.assertTrue(
            (results["gt_bboxes"].numpy() == np.array([[0, 0, 56, 133]])).all()
        )
        ...
        self.assertTrue(
            (results["gt_bboxes"].numpy() == np.array([100, 75, 300, 225])).all()
        )
    ...
    def test_repr(self):
        ...
        self.assertEqual(
            repr(transform),
            (
                "FixShapeResize(width=100, height=50, "
                "keep_ratio=True, "
                "clip_object_border=True), backend=cv2), "
                "interpolation=bilinear)"
            ),
...
class TestRandomFlip(unittest.TestCase):
    def setUp(self):
        ...
        self.results1 = {
            ...
            "gt_masks": BitmapMasks(rng.rand(1, 224, 224), height=224, width=224),
    ...
    def test_transform(self):
        ...
        self.assertTrue(
            (results_update["gt_bboxes"] == np.array([[124, 1, 224, 101]])).all()
        )
        # test only with image
        transform = RandomFlip(1.0)
        results_update = transform.transform(copy.deepcopy(self.results2))
        self.assertTrue((results_update["img"] == self.results2["img"][:, ::-1]).all())
        ...
        self.assertTrue(
            (
                bbox_project(bboxes, results_update["homography_matrix"])
                == results_update["gt_bboxes"]
            ).all()
        )
        # (2) Vertical Flip
    ...
    def test_transform_use_box_type(self):
        ...
        self.assertTrue(
            (
                results_update["gt_bboxes"].numpy() == np.array([[124, 1, 224, 101]])
            ).all()
        )
        ...
        self.assertTrue(
            (
                bbox_project(bboxes, results_update["homography_matrix"])
                == results_update["gt_bboxes"].numpy()
            ).all()
        )
        # (2) Vertical Flip
        transform = RandomFlip(1.0, direction="vertical")
...
class TestPad(unittest.TestCase):
    def setUp(self):
        ...
        self.results = {
            ...
            "gt_masks": BitmapMasks(rng.rand(4, 100, 80), height=100, width=80),
        }
    ...
    def test_repr(self):
        transform = Pad(pad_to_square=True, size_divisor=11, padding_mode="edge")
        self.assertEqual(
            repr(transform),
            (
...
class TestMinIoURandomCrop(unittest.TestCase):
    def test_transform(self):
        ...
        gt_bboxes = create_random_bboxes(
            1, results["img_shape"][1], results["img_shape"][0]
        )
        ...
        self.assertEqual(results["gt_labels"].shape[0], results["gt_bboxes"].shape[0])
    ...
    def test_transform_use_box_type(self):
        ...
        patch = np.array([0, 0, results["img_shape"][1], results["img_shape"][0]])
        ious = bbox_overlaps(
            patch.reshape(-1, 4), results["gt_bboxes"].numpy()
        ).reshape(-1)
        mode = transform.mode
        if mode == 1:
...
class TestSegRescale(unittest.TestCase):
    ...
    def test_repr(self):
        transform = SegRescale(scale_factor=2)
        self.assertEqual(repr(transform), ("SegRescale(scale_factor=2, backend=cv2)"))
...
class TestRandomCrop(unittest.TestCase):
    ...
    def test_transform(self):
        ...
        src_results = {"img": np.random.randint(0, 255, size=(24, 32), dtype=np.int32)}
        target_shape = (12, 16)
        for crop_type, crop_size in zip(
            ["relative", "absolute"], [(0.5, 0.5), (16, 12)]
        ):
        ...
        self.assertTrue(
            (
                bbox_project(bboxes, results["homography_matrix"], (5, 7))
                == results["gt_bboxes"]
            ).all()
        )
        ...
        src_results = {"img": img, "gt_bboxes": gt_bboxes, "gt_masks": gt_masks}
        ...
        results = transform(copy.deepcopy(src_results))
        self.assertTrue((results["gt_bboxes"] == src_results["gt_bboxes"]).all())
    ...
    def test_transform_use_box_type(self):
        ...
        results = transform(copy.deepcopy(src_results))
        self.assertTrue((results["gt_bboxes"].numpy() == target_gt_bboxes).all())
        ...
        results = transform(copy.deepcopy(src_results))
        self.assertTrue(
            (results["gt_bboxes"].numpy() == src_results["gt_bboxes"].numpy()).all()
        )
...
class TestCutOut(unittest.TestCase):
    ...
    def test_transform(self):
        ...
        with self.assertRaises(AssertionError):
            transform = CutOut(n_holes=1, cutout_shape=(2, 2), cutout_ratio=(0.4, 0.4))
        ...
        transform = CutOut(
            n_holes=(2, 4), cutout_shape=[(10, 10), (15, 15)], fill_in=(255, 255, 255)
        )
        results = transform(copy.deepcopy(self.results))
        self.assertTrue(results["img"].sum() > self.results["img"].sum())

        transform = CutOut(n_holes=1, cutout_ratio=(0.8, 0.8), fill_in=(255, 255, 255))
        results = transform(copy.deepcopy(self.results))
        self.assertTrue(results["img"].sum() > self.results["img"].sum())
...
class TestMosaic(unittest.TestCase):
    def setUp(self):
        ...
        self.results = {
            ...
            "gt_bboxes": np.array(
                [[10, 10, 20, 20], [20, 20, 40, 40], [40, 40, 80, 80]], dtype=np.float32
            ),
            "gt_ignore_flags": np.array([0, 0, 1], dtype=bool),
            "gt_masks": BitmapMasks(rng.rand(3, 224, 224), height=224, width=224),
        }
    ...
    def test_transform(self):
        ...
        self.assertTrue(
            results["gt_bboxes_labels"].shape[0] == results["gt_bboxes"].shape[0]
        )
...
class TestRandomAffine(unittest.TestCase):
    ...
    def test_repr(self):
        ...
        self.assertEqual(
            repr(transform),
            (
                "RandomAffine(max_rotate_degree=10.0, "
...
class TestYOLOXHSVRandomAug(unittest.TestCase):
    ...
    def test_transform(self):
        ...
        results = transform(copy.deepcopy(self.results))
        self.assertTrue(results["img"].shape[:2] == self.results["img"].shape[:2])
        self.assertTrue(
            results["gt_bboxes_labels"].shape[0] == results["gt_bboxes"].shape[0]
        )
        self.assertTrue(results["gt_bboxes_labels"].dtype == np.int64)
...
class TestRandomCenterCropPad(unittest.TestCase):
    def test_init(self):
        ...
        with self.assertRaises(AssertionError):
            RandomCenterCropPad(crop_size=(-1, 0), test_mode=False, test_pad_mode=None)
    ...
    def test_transform(self):
        results = dict(img_path=osp.join(osp.dirname(__file__), "../../data/color.jpg"))
...
class TestCopyPaste(unittest.TestCase):
    def setUp(self):
        ...
        self.src_results = {
            ...
            "gt_bboxes_labels": np.ones(src_bboxes.shape[0], dtype=np.int64) * 2,
    ...
    def test_transform(self):
        ...
        results["mix_results"] = [
            {
                "img": self.src_results["img"].copy(),
                "gt_bboxes": self.src_results["gt_bboxes"][valid_inds],
                "gt_bboxes_labels": self.src_results["gt_bboxes_labels"][valid_inds],
                "gt_masks": self.src_results["gt_masks"][valid_inds],
                "gt_ignore_flags": self.src_results["gt_ignore_flags"][valid_inds],
            }
        ]
        results = transform(results)
    ...
    def test_transform_use_box_type(self):
        ...
        results["mix_results"] = [
            {
                "img": src_results["img"].copy(),
                "gt_bboxes": src_results["gt_bboxes"][valid_inds],
                "gt_bboxes_labels": src_results["gt_bboxes_labels"][valid_inds],
                "gt_masks": src_results["gt_masks"][valid_inds],
                "gt_ignore_flags": src_results["gt_ignore_flags"][valid_inds],
            }
        ]
        results = transform(results)
...
class TestAlbu(unittest.TestCase):
    ...
    def test_repr(self):
        ...
        self.assertEqual(
            repr(albu_transform),
            "Albu(transforms=[" "{'type': 'ChannelShuffle', " "'p': 1}])",
        )
...
class TestCorrupt(unittest.TestCase):
    ...
    def test_repr(self):
        ...
        self.assertEqual(
            repr(corrupt_transform), "Corrupt(corruption=gaussian_blur, " "severity=1)"
        )
...
class TestRandomShift(unittest.TestCase):
    ...
    def test_transform(self):
        ...
        results["gt_bboxes_labels"] = np.ones(gt_bboxes.shape[0], dtype=np.int64)
        results["gt_bboxes"] = gt_bboxes
    ...
    def test_repr(self):
        ...
        self.assertEqual(
            repr(transform),
            ("RandomShift(prob=0.5, " "max_shift_px=32, " "filter_thr_px=1)"),
        )
...
class TestRandomErasing(unittest.TestCase):
    ...
    def test_transform(self):
        ...
        transform = RandomErasing(n_patches=1, ratio=0.999, img_border_value=255)
        results = transform(copy.deepcopy(self.results))
        self.assertTrue(results["img"].sum() > self.results["img"].sum())
        # test empty results
        ...
        empty_results["gt_seg_map"] = np.ones_like(empty_results["gt_seg_map"]) * 255
