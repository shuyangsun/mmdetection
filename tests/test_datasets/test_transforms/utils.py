...
def construct_toy_data(poly2mask, use_box_type=False):
    ...
    if poly2mask:
        gt_masks = np.array([[0, 1, 0, 0], [0, 1, 1, 0], [0, 1, 0, 0]], dtype=np.uint8)[
            None, :, :
        ]
    ...
    results["gt_seg_map"] = np.array(
        [[255, 13, 255, 255], [255, 13, 13, 255], [255, 13, 255, 255]], dtype=np.uint8
    )
    return results
...
def check_result_same(results, pipeline_results, check_keys):
    ...
    for key in check_keys:
        ...
        if isinstance(results[key], (BitmapMasks, PolygonMasks)):
            assert_allclose(
                pipeline_results[key].to_ndarray(), results[key].to_ndarray()
            )
