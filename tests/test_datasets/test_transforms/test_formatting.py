...
class TestPackDetInputs(unittest.TestCase):
    ...
    def test_transform(self):
        ...
        self.assertIsInstance(results["data_samples"].proposals.bboxes, torch.Tensor)
    ...
    def test_repr(self):
        transform = PackDetInputs(meta_keys=self.meta_keys)
        self.assertEqual(repr(transform), f"PackDetInputs(meta_keys={self.meta_keys})")
...
class TestPackTrackInputs(unittest.TestCase):
    def setUp(self):
        ...
        self.gt_bboxes_labels = [np.zeros((2,)), np.zeros((2,)) + 1, np.zeros((2,)) - 1]
        ...
        self.img_id = [0, 1, 2]
        self.results_1 = dict(
            img=[self.img.copy(), self.img.copy(), self.img.copy()],
            gt_bboxes=[
                self.gt_bboxes.copy(),
                self.gt_bboxes.copy(),
                self.gt_bboxes.copy(),
        ...
        self.results_2.update(dict(gt_ignore_flags=copy.deepcopy(self.gt_ignore_flags)))
    ...
    def test_transform_without_ignore(self):
        ...
        assert (
            "key_frames_inds" in track_data_sample.metainfo
            and track_data_sample.key_frames_inds == [1]
        )
        assert (
            "ref_frames_inds" in track_data_sample.metainfo
            and track_data_sample.ref_frames_inds == [0, 2]
        )
        for i, data_sample in enumerate(track_data_sample):
            ...
            assert (
                data_sample.gt_instances.labels.numpy() == self.gt_bboxes_labels[i]
            ).all()
            assert (
                data_sample.gt_instances.instances_ids.numpy()
                == self.gt_instances_ids[i]
            ).all()
    ...
    def test_transform_with_ignore(self):
        ...
        for i, data_sample in enumerate(track_data_sample):
            ...
            assert (
                data_sample.gt_instances.labels.numpy()
                == self.gt_bboxes_labels[i][valid_mask]
            ).all()
            assert (
                data_sample.gt_instances.instances_ids.numpy()
                == self.gt_instances_ids[i][valid_mask]
            ).all()
            for key in self.meta_keys:
                assert data_sample.metainfo[key] == getattr(self, key)[i]
...
class TestPackReIDInputs(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.pack_reid_inputs = PackReIDInputs(meta_keys=("flip", "flip_direction"))
    ...
    def test_transform(self):
        ...
        self.assertEqual(data_sample.scale_factor, (1.0, 2.0))
