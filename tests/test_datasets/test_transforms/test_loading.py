...
class TestFilterAnnotations(unittest.TestCase):
    ...
    def test_transform(self):
        ...
        self.assertTrue((results["gt_bboxes_labels"] == np.array([2, 3])).all())
        self.assertTrue(
            (
                results["gt_bboxes"] == np.array([[20, 20, 40, 40], [40, 40, 80, 80]])
            ).all()
        )
...
class TestLoadPanopticAnnotations(unittest.TestCase):
    def setUp(self):
        ...
        self.gt_bboxes = np.array([[0, 0, 10, 5], [0, 5, 5, 10]], dtype=np.float32)
    ...
    def test_transform(self):
        ...
        with patch("panopticapi.utils.rgb2id", mock_rgb2id):
            ...
            transform = LoadPanopticAnnotations(
                with_bbox=False, with_label=False, with_mask=False, with_seg=False
            )
            results = transform(copy.deepcopy(self.results))
            self.assertDictEqual(results, self.results)
            # test with with_mask=True
            transform = LoadPanopticAnnotations(
                with_bbox=False, with_label=False, with_mask=True, with_seg=False
            )
            results = transform(copy.deepcopy(self.results))
            self.assertTrue((results["gt_masks"].masks == self.gt_mask.masks).all())

            # test with with_seg=True
            transform = LoadPanopticAnnotations(
                with_bbox=False, with_label=False, with_mask=False, with_seg=True
            )
            results = transform(copy.deepcopy(self.results))
            ...
            self.assertTrue((results["gt_ignore_flags"] == self.gt_ignore_flags).all())
            self.assertTrue((results["gt_seg_map"] == self.gt_seg_map).all())
...
class TestLoadProposals(unittest.TestCase):
    def test_transform(self):
        ...
        # empty bboxes
        results = {"proposals": dict(bboxes=np.zeros((0, 4), dtype=np.float32))}
        results = transform(results)
    ...
    def test_repr(self):
        transform = LoadProposals()
        self.assertEqual(repr(transform), "LoadProposals(num_max_proposals=None)")
...
class TestLoadEmptyAnnotations(unittest.TestCase):
    def test_transform(self):
        ...
        self.assertEqual(results["gt_masks"].masks.shape[-2:], results["img_shape"])
