...
class DummyDataset(Dataset):
    ...
    def get_data_info(self, idx):
        return dict(
            width=self.shapes[idx][0], height=self.shapes[idx][1], flag=self.flag
        )
...
class TestMultiSourceSampler(TestCase):
    ...
    def test_multi_source_sampler(self):
        ...
        with self.assertRaises(AssertionError):
            MultiSourceSampler(self.dataset_a, batch_size=5, source_ratio=[1, 4])
        # test invalid batch_size
        with self.assertRaises(AssertionError):
            MultiSourceSampler(self.dataset_a, batch_size=-5, source_ratio=[1, 4])
        # test source_ratio longer then dataset
        with self.assertRaises(AssertionError):
            MultiSourceSampler(self.dataset, batch_size=5, source_ratio=[1, 2, 4])
        sampler = MultiSourceSampler(self.dataset, batch_size=5, source_ratio=[1, 4])
...
class TestGroupMultiSourceSampler(TestCase):
    ...
    def test_group_multi_source_sampler(self):
        ...
        flags_gt = ["a", "b", "b", "b", "b"] * 20
        self.assertEqual(flags, flags_gt)
        groups = set([sum(x) for x in (groups[k : k + 5] for k in range(0, 100, 5))])
        groups_gt = set([0, 5])
