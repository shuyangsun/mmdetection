...
class TestDINO(TestCase):
    ...
    def test_dino_head_loss(self):
        ...
        for config in configs:
            ...
            for key, loss in empty_gt_losses.items():
                ...
                if "bbox" in key or "iou" in key or "dn" in key:
                    self.assertEqual(
                        _loss,
                        0,
                        f"there should be no {key}({_loss}) "
                        f"when no ground true boxes",
                    )
                elif "cls" in key:
                    self.assertGreater(_loss, 0, f"{key}({_loss}) should be non-zero")
            ...
            model.eval()
            # test _forward
            model._forward(random_image, batch_data_samples=batch_data_samples_2)
            # test only predict
            model.predict(
                random_image, batch_data_samples=batch_data_samples_2, rescale=True
            )
