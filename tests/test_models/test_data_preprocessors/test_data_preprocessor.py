...
class TestDetDataPreprocessor(TestCase):
    ...
    def test_forward(self):
        ...
        processor = DetDataPreprocessor(
            mean=[0.0, 0.0, 0.0], std=[1.0, 1.0, 1.0], bgr_to_rgb=True
        )
        out_data = processor(data)
        batch_inputs, batch_data_samples = out_data["inputs"], out_data["data_samples"]
        self.assertEqual(batch_inputs.shape, (1, 3, 11, 10))
        ...
        processor = DetDataPreprocessor(
            mean=[0.0, 0.0, 0.0], std=[1.0, 1.0, 1.0], pad_size_divisor=5
        )
        out_data = processor(data)
        batch_inputs, batch_data_samples = out_data["inputs"], out_data["data_samples"]
        self.assertEqual(batch_inputs.shape, (2, 3, 10, 25))
        self.assertEqual(len(batch_data_samples), 2)
        for data_samples, expected_shape in zip(
            batch_data_samples, [(10, 15), (10, 25)]
        ):
        ...
        mask_pad_sums = [
            x.gt_instances.masks.masks.sum() for x in packed_inputs["data_samples"]
        ]
        seg_pad_sums = [
            x.gt_sem_seg.sem_seg.sum() for x in packed_inputs["data_samples"]
        ]
        batch_data_samples = processor(packed_inputs, training=True)["data_samples"]
        for data_samples, expected_shape, mask_pad_sum, seg_pad_sum in zip(
            batch_data_samples, [(10, 24), (10, 24)], mask_pad_sums, seg_pad_sums
        ):
            self.assertEqual(
                data_samples.gt_instances.masks.masks.shape[-2:], expected_shape
            )
            self.assertEqual(data_samples.gt_sem_seg.sem_seg.shape[-2:], expected_shape)
            self.assertEqual(data_samples.gt_instances.masks.masks.sum(), mask_pad_sum)
            self.assertEqual(data_samples.gt_sem_seg.sem_seg.sum(), seg_pad_sum)
    ...
    def test_batch_sync_random_resize(self):
        ...
        self.assertTrue(isinstance(processor.batch_augments[0], BatchSyncRandomResize))
    ...
    def test_batch_fixed_size_pad(self):
        ...
        self.assertTrue(isinstance(processor.batch_augments[0], BatchFixedSizePad))
        packed_inputs = demo_mm_inputs(
            2,
            [[3, 10, 11], [3, 9, 24]],
            with_mask=True,
        ...
        for data_samples, expected_shape, mask_pad_sum, seg_pad_sum in zip(
            batch_data_samples, [(32, 32), (32, 32)], mask_pad_sums, seg_pad_sums
        ):
            self.assertEqual(
                data_samples.gt_instances.masks.masks.shape[-2:], expected_shape
            )
...
class TestMultiBranchDataPreprocessor(TestCase):
    ...
    def test_multi_data_preprocessor(self):
        ...
        self.assertEqual(multi_data["inputs"]["unsup_teacher"].shape, (2, 3, 608, 608))
        self.assertEqual(multi_data["inputs"]["unsup_student"].shape, (2, 3, 704, 704))
