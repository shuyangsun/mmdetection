...
class TestHTCRoIHead(TestCase):
    @parameterized.expand(["point_rend/point-rend_r50-caffe_fpn_ms-1x_coco.py"])
    ...
    def test_point_rend_roi_head_loss(self, cfg_file):
        ...
        for i in range(len(roi_head_cfg.bbox_roi_extractor.featmap_strides)):
            feats.append(
                torch.rand(1, 256, s // (2 ** (i + 2)), s // (2 ** (i + 2))).to(
                    device="cuda"
                )
            )
        feats = tuple(feats)
        ...
        for name, value in out.items():
            if "loss" in name:
                self.assertGreaterEqual(value.sum(), 0, msg="loss should be non-zero")
