...
class TestMaskTrackRCNNTracker(TestCase):
    ...
    def test_get_match_score(self):
        ...
        match_score = self.tracker.get_match_score(
            bboxes, labels, scores, bboxes, labels, similarity_logits
        )
    ...
    def test_track(self, cfg_file):
        ...
        packed_inputs = demo_track_inputs(batch_size=1, num_frames=2, with_mask=True)
        ...
        for frame_id in range(video_len):
            ...
            img_data_sample.pred_instances = img_data_sample.gt_instances.clone()
            # add fake scores
            ...
            feats = []
            for i in range(len(model.track_head.roi_extractor.featmap_strides)):
                feats.append(
                    torch.rand(1, 256, 256 // (2 ** (i + 2)), 256 // (2 ** (i + 2))).to(
                        device="cpu"
                    )
                )
