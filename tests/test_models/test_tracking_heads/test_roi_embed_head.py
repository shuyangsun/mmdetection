...
def _dummy_bbox_sampling(rpn_results_list, batch_gt_instances):
    ...
    for i in range(num_imgs):
        assign_result = bbox_assigner.assign(rpn_results_list[i], batch_gt_instances[i])
        sampling_result = bbox_sampler.sample(
            assign_result, rpn_results_list[i], batch_gt_instances[i], feats=feat
        )
...
class TestRoIEmbedHead(TestCase):
    def test_roi_embed_head_loss(self):
        ...
        num_x_per_img = [1]
        num_x_per_ref_img = [1]
        x_split, ref_x_split = embed_head.forward(
            x, ref_x, num_x_per_img, num_x_per_ref_img
        )
        ...
        rpn_results.priors = torch.Tensor([[23.6667, 23.8757, 238.6326, 151.8874]])
        rpn_results_list = [rpn_results]

        gt_instance = InstanceData()
        gt_instance.labels = torch.LongTensor([2])
        gt_instance.bboxes = torch.Tensor([[23.6667, 23.8757, 238.6326, 151.8874]])
        gt_instance.instances_id = torch.LongTensor([2])
        batch_gt_instances = [gt_instance]

        sampling_results = _dummy_bbox_sampling(rpn_results_list, batch_gt_instances)

        gt_losses = embed_head.loss_by_feat(
            x_split, ref_x_split, sampling_results, gt_instance_ids, ref_gt_instance_ids
        )
        assert gt_losses["loss_match"] > 0, "match loss should be non-zero"
        assert (
            gt_losses["match_accuracy"] >= 0
        ), "match accuracy should be non-zero or zero"
