...
class TestVFNetHead(TestCase):
    def test_vfnet_head_loss(self):
        ...
        s = 256
        img_metas = [
            {"img_shape": (s, s, 3), "scale_factor": 1, "pad_shape": (s, s, 3)}
        ]
        train_cfg = Config(
            dict(
        ...
        empty_gt_losses = vfnet_head.loss_by_feat(
            cls_scores, bbox_preds, bbox_preds_refine, [gt_instances], img_metas
        )
        ...
        one_gt_losses = vfnet_head.loss_by_feat(
            cls_scores, bbox_preds, bbox_preds_refine, [gt_instances], img_metas
        )
