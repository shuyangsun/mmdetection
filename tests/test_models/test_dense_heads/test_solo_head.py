...
class TestSOLOHead(TestCase):
    @parameterized.expand(
        [(SOLOHead,), (DecoupledSOLOHead,), (DecoupledSOLOLightHead,)]
    )
    def test_mask_head_loss(self, MaskHead):
        ...
        for i in range(len(mask_head.strides)):
            feats.append(torch.rand(1, 1, s // (2 ** (i + 2)), s // (2 ** (i + 2))))
        feats = tuple(feats)
        ...
        empty_gt_losses = mask_head.loss_by_feat(
            *mask_outs, batch_gt_instances=[gt_instances], batch_img_metas=img_metas
        )
        ...
        self.assertGreater(onegt_mask_loss.item(), 0, "mask loss should be non-zero")
