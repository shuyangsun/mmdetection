...
class TestRPNHead(TestCase):
    ...
    def test_rpn_head_loss(self):
        ...
        empty_gt_losses = rpn_head.loss_by_feat(
        ...
        self.assertGreater(empty_cls_loss.item(), 0, "rpn cls loss should be non-zero")
        self.assertEqual(
            empty_box_loss.item(),
            0,
        ...
        one_gt_losses = rpn_head.loss_by_feat(
            cls_scores, bbox_preds, [gt_instances], img_metas
        )
        onegt_cls_loss = sum(one_gt_losses["loss_rpn_cls"])
        onegt_box_loss = sum(one_gt_losses["loss_rpn_bbox"])
        self.assertGreater(onegt_cls_loss.item(), 0, "rpn cls loss should be non-zero")
        self.assertGreater(onegt_box_loss.item(), 0, "rpn box loss should be non-zero")
        ...
        with pytest.raises(ValueError):
            rpn_head.loss_by_feat(cls_scores, bbox_preds, [gt_instances], img_metas)
