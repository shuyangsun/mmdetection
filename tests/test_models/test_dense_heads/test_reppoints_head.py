...
class TestRepPointsHead(unittest.TestCase):
    ...
    def test_head_loss(self, transform_method="moment"):
        ...
        x = [torch.rand(1, 32, s // 2 ** (i + 2), s // 2 ** (i + 2)) for i in range(5)]
        ...
        empty_gt_losses = reppoints_head.loss_by_feat(
        ...
        one_gt_losses = reppoints_head.loss_by_feat(
            *forward_outputs, [gt_instances], img_metas, gt_bboxes_ignore
        )
        # loss_cls should all be non-zero
        self.assertTrue(all([loss.item() > 0 for loss in one_gt_losses["loss_cls"]]))
