...
class TestBoxInstHead(TestCase):
    def test_boxinst_maskhead_loss(self):
        ...
        for i in range(len(boxinst_bboxhead.strides)):
            feats.append(torch.rand(1, 1, s // (2 ** (i + 3)), s // (2 ** (i + 3))))
        feats = tuple(feats)
        cls_scores, bbox_preds, centernesses, param_preds = boxinst_bboxhead.forward(
            feats
        )
        ...
        gt_instances.pairwise_masks = (
            _rand_masks(0, gt_instances.bboxes.numpy(), s // 4, s // 4)
            .to_tensor(dtype=torch.float32, device="cpu")
            .unsqueeze(1)
            .repeat(1, 8, 1, 1)
        )
        message_hub = MessageHub.get_instance("runtime_info")
        message_hub.update_info("iter", 1)
        _ = boxinst_bboxhead.loss_by_feat(
            cls_scores, bbox_preds, centernesses, param_preds, [gt_instances], img_metas
        )
        ...
        self.assertEqual(loss_mask_project, 0, "mask project loss should be zero")
        self.assertEqual(loss_mask_pairwise, 0, "mask pairwise loss should be zero")
        ...
        gt_instances.pairwise_masks = (
            _rand_masks(1, gt_instances.bboxes.numpy(), s // 4, s // 4)
            .to_tensor(dtype=torch.float32, device="cpu")
            .unsqueeze(1)
            .repeat(1, 8, 1, 1)
        )
        ...
        self.assertGreater(loss_mask_project, 0, "mask project loss should be nonzero")
        self.assertGreater(
            loss_mask_pairwise, 0, "mask pairwise loss should be nonzero"
        )
