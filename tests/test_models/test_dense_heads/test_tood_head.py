...
class TestTOODHead(TestCase):
    def test_tood_head_anchor_free_loss(self):
        ...
        s = 256
        img_metas = [
            {"img_shape": (s, s, 3), "pad_shape": (s, s, 3), "scale_factor": 1}
        ]
        ...
        empty_gt_losses = tood_head.loss_by_feat(
            cls_scores, bbox_preds, [gt_instances], img_metas, gt_bboxes_ignore
        )
        # When there is no truth, the cls loss should be nonzero but there
        # should be no box loss.
        empty_cls_loss = empty_gt_losses["loss_cls"]
        empty_box_loss = empty_gt_losses["loss_bbox"]
        self.assertGreater(sum(empty_cls_loss).item(), 0, "cls loss should be non-zero")
        self.assertEqual(
            sum(empty_box_loss).item(),
            0,
        ...
        gt_bboxes_ignore = None
        one_gt_losses = tood_head.loss_by_feat(
            cls_scores, bbox_preds, [gt_instances], img_metas, gt_bboxes_ignore
        )
        onegt_cls_loss = one_gt_losses["loss_cls"]
        onegt_box_loss = one_gt_losses["loss_bbox"]
        self.assertGreater(sum(onegt_cls_loss).item(), 0, "cls loss should be non-zero")
        self.assertGreater(sum(onegt_box_loss).item(), 0, "box loss should be non-zero")
