...
class TestCornerHead(TestCase):
    def test_corner_head_loss(self):
        ...
        s = 256
        img_metas = [
            {"img_shape": (s, s, 3), "scale_factor": 1, "batch_input_shape": (s, s, 3)}
        ]
        ...
        feat = [
            torch.rand(1, 1, s // 4, s // 4) for _ in range(corner_head.num_feat_levels)
        ]
        ...
        empty_gt_losses = corner_head.loss_by_feat(
        ...
        self.assertTrue(empty_det_loss.item() > 0, "det loss should be non-zero")
        self.assertTrue(
        ...
        one_gt_losses = corner_head.loss_by_feat(
            *forward_outputs, [gt_instances], img_metas, gt_bboxes_ignore
        )
        ...
        self.assertTrue(onegt_det_loss.item() > 0, "det loss should be non-zero")
        self.assertTrue(
            onegt_push_loss.item() == 0,
            "there should be no push loss when there are only one true box",
        )
        self.assertTrue(onegt_pull_loss.item() > 0, "pull loss should be non-zero")
        self.assertTrue(onegt_off_loss.item() > 0, "off loss should be non-zero")
        ...
        two_gt_losses = corner_head.loss_by_feat(
            *forward_outputs, [gt_instances], img_metas, gt_bboxes_ignore
        )
        ...
        self.assertTrue(twogt_det_loss.item() > 0, "det loss should be non-zero")
        # F.relu limits push loss larger than or equal to 0.
        self.assertTrue(twogt_push_loss.item() >= 0, "push loss should be non-zero")
        self.assertTrue(twogt_pull_loss.item() > 0, "pull loss should be non-zero")
        self.assertTrue(twogt_off_loss.item() > 0, "off loss should be non-zero")
    ...
    def test_corner_head_encode_and_decode_heatmap(self):
        ...
        gt_bboxes = [
            torch.Tensor([[10, 20, 200, 240], [40, 50, 100, 200], [10, 20, 200, 240]])
        ]
        gt_labels = [torch.LongTensor([1, 1, 2])]

        corner_head = CornerHead(num_classes=4, in_channels=1, corner_emb_channels=1)
