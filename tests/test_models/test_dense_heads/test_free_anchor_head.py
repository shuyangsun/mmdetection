...
class TestFreeAnchorRetinaHead(TestCase):
    def test_free_anchor_head_loss(self):
        ...
        empty_gt_losses = anchor_head.loss_by_feat(
        ...
        self.assertGreater(
            negative_bag_loss.item(), 0, "negative_bag loss should be non-zero"
        )
        self.assertEqual(
            positive_bag_loss.item(),
            0,
        ...
        one_gt_losses = anchor_head.loss_by_feat(
            cls_scores, bbox_preds, [gt_instances], img_metas
        )
        onegt_cls_loss = one_gt_losses["positive_bag_loss"]
        onegt_box_loss = one_gt_losses["negative_bag_loss"]
        self.assertGreater(
            onegt_cls_loss.item(), 0, "positive bag loss should be non-zero"
        )
        self.assertGreater(
            onegt_box_loss.item(), 0, "negative bag loss should be non-zero"
        )
