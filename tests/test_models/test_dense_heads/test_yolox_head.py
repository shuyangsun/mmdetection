...
class TestYOLOXHead(TestCase):
    def test_init_weights(self):
        ...
        for conv_cls, conv_obj in zip(
            head.multi_level_conv_cls, head.multi_level_conv_obj
        ):
            assert_allclose(
                conv_cls.bias.data, torch.ones_like(conv_cls.bias.data) * bias_init
            )
            assert_allclose(
                conv_obj.bias.data, torch.ones_like(conv_obj.bias.data) * bias_init
            )
    ...
    def test_predict_by_feat(self):
        ...
        feat = [
            torch.rand(1, 1, s // feat_size, s // feat_size) for feat_size in [4, 8, 16]
        ]
    ...
    def test_loss_by_feat(self):
        ...
        self.assertGreater(
            empty_obj_loss.item(), 0, "objectness loss should be non-zero"
        )
        ...
        assert isinstance(
            head.multi_level_cls_convs[0][0], DepthwiseSeparableConvModule
        )
        head.use_l1 = True
        gt_instances = InstanceData(
        ...
        one_gt_losses = head.loss_by_feat(
            cls_scores, bbox_preds, objectnesses, [gt_instances], img_metas
        )
