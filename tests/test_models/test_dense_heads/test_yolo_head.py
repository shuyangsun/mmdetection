...
class TestYOLOV3Head(TestCase):
    def test_yolo_head_loss(self):
        ...
        empty_gt_losses = head.loss_by_feat(predmaps, [gt_instances], img_metas)
        ...
        self.assertEqual(
            empty_xy_loss, 0, "there should be no xy loss when there are no true boxes"
        )
        self.assertEqual(
            empty_wh_loss, 0, "there should be no wh loss when there are no true boxes"
        )
