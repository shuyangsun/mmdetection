...
def test_filter_gt_instances():
    ...
    for inputs in packed_inputs:
        inputs.gt_instances.scores = torch.ones_like(inputs.gt_instances.labels).float()
...
def test_rename_loss_dict():
    prefix = "sup_"
    losses = {"cls_loss": torch.tensor(2.0), "reg_loss": torch.tensor(1.0)}
