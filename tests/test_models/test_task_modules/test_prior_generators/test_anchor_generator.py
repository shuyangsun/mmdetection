...
def test_sparse_prior():
    ...
    mlvl_anchors = AnchorGenerator(
        strides=[16, 32], ratios=[1.0], scales=[1.0], base_sizes=[4, 8]
    )
    prior_indexs = torch.Tensor([0, 2, 4, 5, 6, 9]).long()
    ...
    if torch.cuda.is_available():
        ...
        mlvl_anchors = AnchorGenerator(
            strides=[16, 32], ratios=[1.0, 2.5], scales=[1.0, 5.0], base_sizes=[4, 8]
        )
...
def test_standard_anchor_generator():
    ...
    anchor_generator_cfg = dict(
        type="AnchorGenerator", scales=[8], ratios=[0.5, 1.0, 2.0], strides=[4, 8]
    )
