...
class TestHungarianAssigner(TestCase):
    ...
    def test_hungarian_match_assigner(self):
        ...
        assign_result = assigner.assign(pred_instances, gt_instances, img_meta=img_meta)
        ...
        self.assertEqual((assign_result.gt_inds > 0).sum(), gt_instances.bboxes.size(0))
        self.assertEqual((assign_result.labels > -1).sum(), gt_instances.bboxes.size(0))
    ...
    def test_bbox_match_cost(self):
        ...
        # test IoUCost
        assigner = HungarianAssigner(ConfigDict(dict(type="IoUCost", iou_mode="iou")))
        assign_result = assigner.assign(pred_instances, gt_instances, img_meta=img_meta)
        self.assertTrue(torch.all(assign_result.gt_inds > -1))
        self.assertEqual((assign_result.gt_inds > 0).sum(), gt_instances.bboxes.size(0))
        self.assertEqual((assign_result.labels > -1).sum(), gt_instances.bboxes.size(0))
    ...
    def test_mask_match_cost(self):
        ...
        assign_result = assigner.assign(pred_instances, gt_instances, img_meta=img_meta)
        self.assertTrue(torch.all(assign_result.gt_inds > -1))
        self.assertEqual((assign_result.gt_inds > 0).sum(), gt_instances.masks.size(0))
        self.assertEqual((assign_result.labels > -1).sum(), gt_instances.masks.size(0))
        ...
        # test FocalLossCost
        assigner = HungarianAssigner(dict(type="FocalLossCost", binary_input=True))
        assign_result = assigner.assign(pred_instances, gt_instances, img_meta=img_meta)
        self.assertTrue(torch.all(assign_result.gt_inds > -1))
        self.assertEqual((assign_result.gt_inds > 0).sum(), gt_instances.masks.size(0))
        self.assertEqual((assign_result.labels > -1).sum(), gt_instances.masks.size(0))
