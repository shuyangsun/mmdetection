...
def test_dilated_encoder():
    ...
    out_channels = 32
    out_shape = 34
    dilated_encoder = DilatedEncoder(in_channels, out_channels, 16, 2, [2, 4, 6, 8])
...
def test_ssd_neck():
    ...
    with pytest.raises(AssertionError):
        SSDNeck(
            in_channels=[8, 16], out_channels=[8], level_strides=[2], level_paddings=[2]
        )
...
def test_ssh_neck():
    ...
    ssh_model = SSH(num_scales=4, in_channels=in_channels, out_channels=out_channels)
