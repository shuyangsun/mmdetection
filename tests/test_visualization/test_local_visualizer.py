...
class TestDetLocalVisualizer(TestCase):
    def test_add_datasample(self):
        ...
        # test out_file
        det_local_visualizer.add_datasample(
            "image", image, det_data_sample, draw_pred=False, out_file=out_file
        )
...
class TestTrackLocalVisualizer(TestCase):
    ...
    def _get_pred_instances():
        instances_data = dict(
            bboxes=torch.tensor([[900, 500, 1000, 600], [1300, 400, 1500, 800]]),
            instances_id=torch.tensor([1, 2]),
            labels=torch.tensor([0, 1]),
    ...
    def test_add_datasample(self):
        ...
        track_local_visualizer.dataset_meta = dict(classes=["pedestrian", "vehicle"])

        # test gt_instances
        track_local_visualizer.add_datasample("image", image, image_data_sample, None)
        ...
        track_local_visualizer.add_datasample(
            "image", image, image_data_sample, draw_gt=False, out_file=out_file
        )
        self._assert_image_and_shape(out_file, (h, w, 3))

        track_local_visualizer.add_datasample(
            "image", image, image_data_sample, draw_pred=False, out_file=out_file
        )
        self._assert_image_and_shape(out_file, (h, w, 3))
