...
class TestInferenceBenchmark(unittest.TestCase):
    ...
    @unittest.skipIf(not torch.cuda.is_available(), "test requires GPU and torch+cuda")
    def test_init_and_run(self):
        ...
        inference_benchmark = InferenceBenchmark(
            cfg, checkpoint_path, False, False, self.max_iter, self.log_interval
        )
        ...
        cfg = copy.deepcopy(self.cfg)
        cfg.env_cfg.cudnn_benchmark = True
        cfg.env_cfg.mp_cfg = {"mp_start_method": "fork", "opencv_num_threads": 1}
        inference_benchmark = InferenceBenchmark(
            cfg, checkpoint_path, False, False, self.max_iter, self.log_interval
        )
        inference_benchmark.run(1)
        ...
        # test logger
        logger = MMLogger.get_instance("mmdet", log_file="temp.log", log_level="INFO")
        inference_benchmark = InferenceBenchmark(
            cfg,
            checkpoint_path,
            False,
...
class TestDataLoaderBenchmark(unittest.TestCase):
    ...
    def test_init_and_run(self):
        cfg = copy.deepcopy(self.cfg)
        dataloader_benchmark = DataLoaderBenchmark(
            cfg, False, "train", self.max_iter, self.log_interval, self.num_warmup
        )
        ...
        dataloader_benchmark = DataLoaderBenchmark(
            cfg, False, "val", self.max_iter, self.log_interval, self.num_warmup
        )
        ...
        dataloader_benchmark = DataLoaderBenchmark(
            cfg, False, "test", self.max_iter, self.log_interval, self.num_warmup
        )
        self.assertTrue(dataloader_benchmark.data_loader.num_workers == 1)
...
class TestDatasetBenchmark(unittest.TestCase):
    ...
    def test_init_and_run(self):
        ...
        dataset_benchmark = DatasetBenchmark(
            cfg, "train", self.max_iter, self.log_interval, self.num_warmup
        )
        ...
        # test test dataset
        dataset_benchmark = DatasetBenchmark(
            cfg, "test", self.max_iter, self.log_interval, self.num_warmup
        )
        dataset_benchmark.run(1)

        # test val dataset
        dataset_benchmark = DatasetBenchmark(
            cfg, "val", self.max_iter, self.log_interval, self.num_warmup
        )
        dataset_benchmark.run(1)
        ...
        with self.assertRaises(AssertionError):
            DatasetBenchmark(
                cfg, "training", self.max_iter, self.log_interval, self.num_warmup
            )
